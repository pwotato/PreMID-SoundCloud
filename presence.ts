...
presence.on('UpdateData', async () => {
  ...
  if ((playing || (!playing && !showBrowsing)) && showSong) {
    ...
    if (usePresenceArtist && !usePresenceTitle) {
      presenceData.name = getElement('.playbackSoundBadge__lightLink')
    }
    else {
	  presenceData.name = getElement('.playbackSoundBadge__titleLink > span:nth-child(2)') + ' by ' + getElement('.playbackSoundBadge__lightLink')
    }
